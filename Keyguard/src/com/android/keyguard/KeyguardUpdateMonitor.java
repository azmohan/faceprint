...
import com.freeme.face.ui.FaceprintCommand;
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
     ...
     */
    /*/freeme,gouzhouping.20160830,for decrease the time after fingerprint times out.
    private static final long FINGERPRINT_UNLOCK_TIMEOUT_MS = 72 * 60 * 60 * 1000;
    /*/
    private static final long FINGERPRINT_UNLOCK_TIMEOUT_MS = 24 * 60 * 60 * 1000;
    //*/
    ...
    //*/freeme.chenming, 20160802. unlock after fingerprint if keyguard occlued
...
//*/ modify by Dori 20160118 for customize keyguard
   /* public static class BatteryStatus {
    ...
    private KeyguardUpdateMonitor(Context context) {
        ...
        Intent intent = context.registerReceiver(mFaceStateChangeReceiver,
                new IntentFilter(FaceprintCommand.ACTION_FACEPRINT_STATE));
        mHasEnrolledFace = intent != null
                && intent.getBooleanExtra(FaceprintCommand.EXTRA_HAS_ENROLLED_FACE, false);
        mHasAuthenticateUI = intent != null
                && intent.getBooleanExtra(FaceprintCommand.EXTRA_HAS_AUTHENTICATE_UI, false);
        //*/
    }
...
/*        int resultId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
    ...
    */
    ...
    //*/ freeme,chenming. 20171218. Faceprint
    private boolean mHasEnrolledFace;
    private boolean mHasAuthenticateUI;
    private boolean mHasAuthenticateSuccess;

    private final BroadcastReceiver mFaceStateChangeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            final int faceState =
                    intent.getIntExtra(FaceprintCommand.EXTRA_STATUS, FaceprintCommand.STATUS_AUTHENTICATE_STOP);
            mHasAuthenticateSuccess =
                    faceState == FaceprintCommand.STATUS_AUTHENTICATE_SUCCESS;
            mHasEnrolledFace =
                    intent.getBooleanExtra(FaceprintCommand.EXTRA_HAS_ENROLLED_FACE, false);
            mHasAuthenticateUI =
                    intent.getBooleanExtra(FaceprintCommand.EXTRA_HAS_AUTHENTICATE_UI, false);

            handleFaceUnlockStateChanged(faceState == FaceprintCommand.STATUS_AUTHENTICATE_START, getSendingUserId());
        }
    };

    public boolean hasEnrolledFace() {
        return mHasEnrolledFace;
    }

    public boolean hasAuthenticateUI() {
        return mHasAuthenticateUI;
    }

    public boolean hasAuthenticateSuccess() {
        return mHasAuthenticateSuccess;
    }

    public void updateFaceService(boolean started) {
        Log.d(TAG, "cancelFaceService()");
        FaceprintCommand.send(mContext, started ?
                FaceprintCommand.CMD_AUTHENTICATE_START :
                FaceprintCommand.CMD_AUTHENTICATE_STOP);
    }
   //*/
}
