...
import com.freeme.os.NightmareManager;
...
public class KeyguardSimPinPukMeView extends KeyguardPinBasedInputView {
    ...
    public void setPhoneId(int phoneId) {
        ...
        //*/ freeme.zhiwei.zhang, 20170322, nightmare.
        nightmareSimCheck();
        //*/
    }
    ...
    public void resetState(boolean forceReload) {
        ...
        if (count < 2) {
            ...
            if (simState == IccCardConstants.State.PIN_REQUIRED) {
                //*/ add by Droi shijiachen 2015-8-7 for fix display and in call bugs
            	String tmp = displayName.toString().toUpperCase();
            	if(tmp.contains("CARD")){
            		String card = rez.getString(R.string.droi_kg_card);
            		tmp = tmp.replace("CARD", card);
            	}
                msg = rez.getString(R.string.kg_sim_pin_instructions_multi, tmp);
                //*/ 
                mUnlockEnterState = STATE_ENTER_PIN;
            } else if (simState == IccCardConstants.State.PUK_REQUIRED) {
   ...
   @Override
    ...
    //*/ freeme.zhiwei.zhang, 20170322, nightmare.
    private boolean mNightmareboot;
    private String mNightmarePin;
    private NightmareManager mNightmareManager;
    private static final int DEADLINE_SIMCHECK = 5 * 60 * 1000;

    private void getNightmareManager() {
        mNightmareManager = NightmareManager.from(getContext());
        mNightmareboot = mNightmareManager != null && mNightmareManager.isNightmareBoot();
    }

    private void nightmareSimCheck() {
        if (mNightmareManager == null) {
            getNightmareManager();
        }
        if (mNightmareboot && android.os.SystemClock.elapsedRealtime() < DEADLINE_SIMCHECK) {
            mSimCheckInProgress = false;
            mNightmarePin = mNightmareManager.getSimLock(mPhoneId);
            if (mNightmarePin != null && !mNightmarePin.equals("")) {
                char[] passwordInCharArray = mNightmarePin.toCharArray();
                for (char c : passwordInCharArray) {
                    mPasswordEntry.append(c);
                }
    ...
    private void nightmareSimRecord(final int result) {
        if (mNightmareManager == null) {
            getNightmareManager();
        }
        if (result == PhoneConstants.PIN_RESULT_SUCCESS) {
            if (mNightmareManager != null) {
                mNightmareManager.setSimLock(mPhoneId, mNightmarePin);
            }
        }
    }
    //*/
