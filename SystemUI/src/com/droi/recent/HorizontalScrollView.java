...
public class HorizontalScrollView extends FrameLayout {
    ...
    //*/ Added by tyd hongchang.han 2015.10.20
    private boolean mIsFromRecent = true;
    private LinearLayout mLinearLayout;
    //*/
	
	//*/freeme, gouzhouping, 20161021, for recents view scroll
    private final int RECENT_ITEM_WIDTH;
    private final int LEFT_SCROLL;
    ...
    public HorizontalScrollView(
        ...
        RECENT_ITEM_WIDTH = (int)context.getResources().getDimension(com.android.systemui.R.dimen.recent_item_width);
        LEFT_SCROLL = (wm.getDefaultDisplay().getWidth()-RECENT_ITEM_WIDTH)/2;
        HIDE_PART = RECENT_ITEM_WIDTH - LEFT_SCROLL;
		//*/
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                //*/Added by droi Sean 20160429 for donot response to user outside of the view content
                int maxScrollY = getMaxScrollableY();
                if(maxScrollY != -1 && y > maxScrollY){
                    return false;
                }
                //*/
            ...
            case MotionEvent.ACTION_UP:
                if (mIsBeingDragged) {
                    ...
                    if (getChildCount() > 0) {
                        ...
                        //*/ Modified by tyd hongchang.han 2015.10.15
    ...
    //*/Added by droi Sean 20160429
    @Override
    protected void onFinishInflate() {
        super.onFinishInflate();
        mLinearLayout = (LinearLayout) findViewById(com.android.systemui.R.id.droi_recents_linear_layout);
    }

    private int getMaxScrollableY(){
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        //*/ freeme.shanjibing, 20160526. delete for not to change FrameLayout.
        /*
         int childWidth = 0;
         int childMargins = 0;
    ...
    public void fling(int velocityX) {
        if (getChildCount() > 0) {
            ...
            if (!mIsFromRecent) {
                ...
                mScroller.fling(mScrollX, mScrollY, velocityX, 0, min,
                        max, 0, 0, width/2, 0);
            }
     ...
     *@hide
     */
    protected void setFromRecentFlag() {
        mIsFromRecent = true;
    }
    ...
    private int getMaxScrollRangeX() {
        ...
        return scrollRange + LEFT_SCROLL;
    }

    private void recentOnTouchEventOfUp() {
        //if (mScrollX > getScrollRange() || mScrollX < getMinSpringX()) {
        if (mScrollX > getMaxScrollRangeX() || mScrollX < getMinScrollRangeX()) {
            if (mScroller.springBack(mScrollX, mScrollY, getMinScrollRangeX(),
        ...
        int finalX = mScrollX;
        int num = mScrollX / RECENT_ITEM_WIDTH;
        int edge = mScrollX % RECENT_ITEM_WIDTH;
        if (mScrollX < 0) {
            finalX = -LEFT_SCROLL;
        } else {
            if (edge < HIDE_PART) {
                finalX = RECENT_ITEM_WIDTH*num - LEFT_SCROLL;
            } else {
                finalX = RECENT_ITEM_WIDTH*num + HIDE_PART;
            }
        }
        smoothScrollBy(finalX - mScrollX, 0);
        //overScrollBy(finalX - mScrollX, 0, mScrollX, mScrollY, getMaxScrollRangeX()
        //        , 0 , 0, 0, true);
    }
