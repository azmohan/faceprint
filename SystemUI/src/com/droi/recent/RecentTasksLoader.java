...
public class RecentTasksLoader implements View.OnTouchListener {
    ...
    /*/ freeme.Jack, 20160722, for tasks count (display 8)
    private static final int DISPLAY_TASKS = 20;
    private static final int MAX_TASKS = DISPLAY_TASKS + 1; // allow extra for non-apps
    /*/
    private static final int DISPLAY_TASKS = SystemProperties.getInt("persist.sys.display_tasks", 8);
    private static final int MAX_TASKS = DISPLAY_TASKS + 10; // allow extra for non-apps
    //*/
    //*/ freeme.gouzhouping, 20160620, for recent tasks show
    private static final String CANCEL_RECENT_TASKS_NAME = "com.zhuoyi.security.service";
    //*/
    ...
    //Added by Sean 2015-12-09 for all the recents tasks begin
    //backup list
    private ArrayList<TaskDescription> mAllTasks = new ArrayList<TaskDescription>();
    //Added by Sean 2015-12-09 for all the recents tasks end
    ...
    public void remove(TaskDescription td) {
        ...
        //*/ Added by Sean 2015-12-09 for all the recents tasks
        mAllTasks.remove(td);
        //*/
    }
    ...
    void loadThumbnailAndIcon(TaskDescription td) {
        ...
        //*/Added by Droi Sean 2015-12-04 for updating app icon
        Drawable icon = null;
        try {
            icon = td.resolveInfo.loadIcon(pm);
        }catch(Exception e){
            icon = getFullResIcon(td.resolveInfo, pm);
        }
        //*/
    ...
    //*/ Added by Sean 2015-12-09 for clearup all tasks
    private void loadAllTasks(ActivityManager am, ActivityInfo homeInfo){
        if(mAllTasks != null){
            mAllTasks.clear();
        }
    ...
    public void clearUpRecentTasks(ArrayList<String> whitePackages){
        if (mAllTasks != null && mAllTasks.size() != 0) {
            final ActivityManager am = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
            for (int i=0; i<mAllTasks.size(); i++) {
                if (!whitePackages.contains(mAllTasks.get(i).packageName)) {
                    if (am != null) {
                        am.removeTask(mAllTasks.get(i).persistentTaskId);
                    }
                }
            }
        }
    ...
    //*/ freeme.Jack 20160727, remove outermost task when display task
    private void removeOutermostTask(ArrayList<TaskDescription> hold, ArrayList<TaskDescription> all) {
        ArrayList<Integer> holdList = new ArrayList<Integer>();
        for (TaskDescription task : hold) {
            holdList.add(task.persistentTaskId);
        }
        final ActivityManager am = (ActivityManager)
                mContext.getSystemService(Context.ACTIVITY_SERVICE);
        for (TaskDescription task : all) {
            if (!holdList.contains(task.persistentTaskId))
                am.removeTask(task.persistentTaskId);
        }
    }
    //*/
    ...
    public void loadTasksInBackground(final boolean zeroeth) {
        /*/ freeme.Jack 20160727, fixed list display empty
        ...
        mTaskLoader = new AsyncTask<Void, ArrayList<TaskDescription>, Void>() {
            ...
            protected void onPostExecute(Void result) {
                ...
                if (FeatureOption.FREEME_EXTREME_LK_TRIM)
                    removeOutermostTask(mLoadedTasks, mAllTasks);
            }
            //*/
            ...
            protected Void doInBackground(Void... params) {
                ...
                /*/
                for (int i = first, index = 0; i < numTasks && (index < DISPLAY_TASKS); ++i) {
                //*/
                    if (isCancelled()) {
                        break;
                    }
                    ...
                    //*/ freeme,gouzhouping. 20160620, for recent tasks show except "com.zhuoyi.security.service"
                    if (intent.getComponent().getPackageName().equals(CANCEL_RECENT_TASKS_NAME)) {
                    	continue;
                    }
                    //*/
                ...
                //Added by Sean 2015-12-09 for load all tasks begin
                loadAllTasks(am, homeInfo);
                //Added by Sean 2015-12-09 for load all tasks end
