...
public class WifiTile extends QSTile<QSTile.SignalState> {
    ...
    //*/ Added by droi hanhao for customized QSTile 2015-07-13
    @Override
    protected void handleLongClick() {
        Intent intent = new Intent(android.provider.Settings.ACTION_WIFI_SETTINGS);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.setPackage("com.android.settings"); // set package explicitly
    ...
    protected void handleUpdateState(SignalState state, Object arg) {
        ...
        state.selected = cb.enabled;
        if(CUSTOMIZED_QSTILE) {
            if(!state.enabled) {
                state.icon = ResourceIcon.get(R.drawable.droi_ic_qs_wlan_off);
            } else if(wifiConnected) {
                state.icon = ResourceIcon.get(R.drawable.droi_ic_qs_wlan_connected);
            } else if (wifiNotConnected) {
                state.icon = ResourceIcon.get(R.drawable.droi_ic_qs_wlan_no_connected);
            } else {
                state.icon = ResourceIcon.get(R.drawable.droi_ic_qs_wlan_no_network);
            }
        }
        //*/
