...
public class AirplaneModeTile extends QSTile<QSTile.BooleanState> {
    ...
    private final int mEnableDrawableId = R.drawable.droi_ic_qs_tile_airplane_checked;
    private final int mDisableDrawableId = R.drawable.droi_ic_qs_tile_airplane_normal;
    //*/
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        state.selected = airplaneMode;
        //*/
        state.value = airplaneMode;
        state.visible = true;
        state.label = mContext.getString(R.string.airplane_mode);
        //*/ Modified by droi hanhao for customized QSTile 2015-06-24
        if (airplaneMode) {
            state.icon = CUSTOMIZED_QSTILE ? ResourceIcon.get(mEnableDrawableId) : mEnable;
            state.contentDescription =  mContext.getString(
                    R.string.accessibility_quick_settings_airplane_on);
        } else {
            state.icon = CUSTOMIZED_QSTILE ? ResourceIcon.get(mDisableDrawableId) : mDisable;
            state.contentDescription =  mContext.getString(
