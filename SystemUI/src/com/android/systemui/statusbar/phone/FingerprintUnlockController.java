...
public class FingerprintUnlockController extends KeyguardUpdateMonitorCallback {
    ...
    private void releaseFingerprintWakeLock() {
        if (mWakeLock != null) {
            ...
            //*/ freeme, gouzhouping, 20161220, for reset the state when released.
            isPowerWakeUp = true;
            //*/
            mWakeLock.release();
            mWakeLock = null;
    ...
    public void onFingerprintAuthenticated(int userId) {
        ...
        switch (mMode) {
            ...
            case MODE_SHOW_BOUNCER:
                ...
                /*/
                if(mPhoneStatusBar.mNotificationPanel.isDefaultLockscreen()){
                    if (!wasDeviceInteractive) {
                        mStatusBarKeyguardViewManager.notifyDeviceWakeUpRequested();
                    }
                mStatusBarKeyguardViewManager.animateCollapsePanels(
                    ...
                    mStatusBarKeyguardViewManager.dismissDoriKeyguardForFingerPrint(false);
                }
                //*/                 
                break;
            ...
            case MODE_WAKE_AND_UNLOCK:
                ...
                mDozeScrimController.abortPulsing();
                mKeyguardViewMediator.onWakeAndUnlocking();
                //*/freeme.shanjibing, 20160901. optimized: delete the invalidate for scrim
    ...
    //*/ freeme, gouzhouping, 20161216, for control the fp error power on
    private boolean isPowerWakeUp = true;
    //*/

    @Override
    public void onFingerprintError(int msgId, String errString) {
        //*/ freeme, gouzhouping, 20161216, for enter into security surface when finger print error
        if(msgId == FingerprintManager.FINGERPRINT_ERROR_LOCKOUT) {
            //*/droi.dukanglei, 20170426 , for Emergency call when fingerprint error
            if(!mUpdateMonitor.isDeviceInteractive()) {
                mStatusBarKeyguardViewManager.notifyDeviceWakeUpRequested();
            }
            ...
            //*/
            if(FreemeFeature.isLocalSupported("FINGERPRINT.FAIL.WAKEUP") && isPowerWakeUp) {
                isPowerWakeUp = false;
            	mPowerManager.wakeUp(SystemClock.uptimeMillis());
            }
        }
