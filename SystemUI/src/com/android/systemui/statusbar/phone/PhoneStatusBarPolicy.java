...
//*/Added by tyd Greg 2014-05-26,for visitor mode
import android.database.ContentObserver;
//*/
//*/ Added by Linguanrong for display float gesture icon, 2013-10-31
...
public class PhoneStatusBarPolicy implements Callback {
    ...
    //*/ Added by Linguanrong for display float gesture icon, 2013-10-31
    private static final String FLOAT_GESTURE = "com.freeme.action.FLOAT_GESTURE";
    private boolean gesGenModeChecked = false;
    private static boolean mIsGestureOn = false;
    //*/
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// M: [Multi-User] Register Alarm intent by user @}
            //*/ Added by Linguanrong for display float gesture icon, 2013-10-31
            else if(FLOAT_GESTURE.equals(action)) {
                updateFloatGesture(intent);
            }
            //*/
        }
    };
    ...
    public PhoneStatusBarPolicy(Context context, CastController cast, HotspotController hotspot,
        ...
        //*/ Added by Linguanrong for display float gesture icon, 2013-10-31
        filter.addAction(FLOAT_GESTURE);
        ...
        //*/ Added by Linguanrong for display float gesture icon, 2013-10-31
        mService.setIcon("float_gesture", R.drawable.droi_stat_sys_float_gesture, 0, null);
        mService.setIconVisibility("float_gesture", isGestureModeOn());
        //*/

        //*/Added by tyd Greg 2014-05-26,for visitor mode
        mService.setIcon("visitor_mode", R.drawable.droi_stat_sys_data_visitor_mode, 0, null);
        mService.setIconVisibility("visitor_mode", false);
        mMyModeObserver.observe();
        //*/
    }
    ...
    private final void updateVolumeZen() {
        ...
        if (DndTile.isVisible(mContext) || DndTile.isCombinedIcon(mContext)) {
            ...
            zenVisible = true;
            //*/ Modified by tyd hanhao 2015-08-01
            // This mode is the same as silent profile, so we use 'ringer_silent' repleace 'sys_zen_none' icon
            zenIconId = R.drawable.droi_stat_sys_ringer_silent;//R.drawable.stat_sys_zen_none;
            //*/
        ...
        //*/ Modified by droi hanhao for not show zenicon in statusview 2016-01-30
        zenVisible = false;
        //*/
        ...
        if (DndTile.isVisible(mContext) && !DndTile.isCombinedIcon(mContext)
            ...
            volumeVisible = true;
            //*/ Modified by tyd hanhao, customized icon 2015-08-01
            volumeIconId = R.drawable.droi_stat_sys_ringer_silent;
            //*/
            ...
            volumeVisible = true;
            //*/ Modified by tyd hanhao, customized icon 2015-08-01
            volumeIconId = R.drawable.droi_stat_sys_ringer_vibrate;
            //*/
   ...
   //*/ Added by Linguanrong for display float gesture icon, 2013-10-31
    private final void updateFloatGesture(Intent intent) {
        if(!isGestureModeOn()) {
            boolean on = intent.getBooleanExtra("isFloatGestureOn", false);
            boolean fromKeyguard = intent.getBooleanExtra("isFromKeyguard", false);
            
            if(mIsGestureOn == false || on == true || !fromKeyguard) {
                mService.setIconVisibility("float_gesture", on);
            }
            ...
            mService.setIconVisibility("float_gesture", gesGenModeChecked);
        }
    }

    private static int tryGestureModeCount = 0;
    private boolean isGestureModeOn() {
        INativeMiscService mNativeMiscService = INativeMiscService.Stub.asInterface(
                android.os.ServiceManager.getService(INativeMiscService.SERVICE_NAME));
        try {
            gesGenModeChecked = (mNativeMiscService.devStateCtrl(INativeMiscService.DSC_DEV_GES_GEN,
                INativeMiscService.DSC_OPR_FETCH, 0) == INativeMiscService.DSC_MODE_ENABLE);
            tryGestureModeCount = 0;
        } catch (NullPointerException e) {
            gesGenModeChecked = false;
            if (mHandler != null) {
                mHandler.postDelayed(new Runnable() {
                    public void run() {
                        tryGestureModeCount++;
                        if(tryGestureModeCount <= 3) {
                            mService.setIconVisibility("float_gesture", isGestureModeOn());
                        } else {
                            tryGestureModeCount = 0;
                        }
        ...
        return gesGenModeChecked;
    }
    //*/

    //*/Added by tyd Greg 2014-05-26,for visitor mode
    private VisitorContentObserver mMyModeObserver = new VisitorContentObserver(new Handler());
    private static final String DEBUG_TAG = "VisitorMode";
    class VisitorContentObserver extends ContentObserver {

        public VisitorContentObserver(Handler handler) {
            super(handler);
        }

        public void onChange(boolean selfChange) {
            //android.util.Log.i(DEBUG_TAG, "onChange:  " + selfChange);
            updateVisitorIcon();
        }

        public void observe() {
            //android.util.Log.i(DEBUG_TAG, "observe");
            mContext.getContentResolver().registerContentObserver(Settings.System.getUriFor("vistor_mode_status_state"), false, this);
            updateVisitorIcon();
        }
    }

    private void updateVisitorIcon() {
        boolean stateOn = Settings.System.getInt(mContext.getContentResolver(), "vistor_mode_status_state", 0) == 1;
        mService.setIcon("visitor_mode", R.drawable.droi_stat_sys_data_visitor_mode, 0, null);
        mService.setIconVisibility("visitor_mode", stateOn);
        //android.util.Log.i(DEBUG_TAG, "updateVisitor :" + stateOn);
    }
    //*/
}
