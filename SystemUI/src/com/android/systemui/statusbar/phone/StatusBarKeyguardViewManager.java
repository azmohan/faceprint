...
public class StatusBarKeyguardViewManager {
    ...
    private DroiOnDismissAction mAfterKeyguardGoneAction;
    ...
    //*/
    public void hideBouncer(boolean isDestoryView){
    	mBouncer.hide(isDestoryView);
    }

    public void dismissWithAction(DroiOnDismissAction r, Runnable cancelAction,
    ...
    //*/add by Dori fenglei 2016-3-25 for Keyguard customize
    public void dismissDoriKeyguard(boolean authenticated) {
        if (DEBUG) Log.d(TAG, "dismissDoriKeyguard(authenticated = " + authenticated + ") is called." +
             " mScreenOn = " + mDeviceInteractive) ;
        // VoiceWakeup will need to dismiss keyguard even if screen is off.
        if (authenticated || mDeviceInteractive || VoiceWakeupManager.getInstance().isDismissAndLaunchApp()) {
            showBouncer(authenticated);
        }
    }
    //*/

    //*/freeme.shanjibing, 20160830. support for third lockscreen fingerprint unlock
    public void dismissDoriKeyguardForFingerPrint(boolean authenticated) {
        if (DEBUG) Log.d(TAG, "dismissDoriKeyguard(authenticated = " + authenticated + ") is called." +
             " mScreenOn = " + mDeviceInteractive) ;
        showBouncer(authenticated);
    }
    ...
    public void updateStates() {
        ...
        if (com.droi.feature.FeatureOption.FREEME_NAVIGATIONBAR_MIN) {
            navBarVisible = navBarVisible && android.provider.Settings.System.getInt(mContext.getContentResolver(),
                    android.provider.Settings.System.FREEME_NAVIGATIONBAR_IS_MIN, 0) == 0;
        }
        //*/
        ...
        if ((showing && !occluded) != (mLastShowing && !mLastOccluded) || mFirstUpdate) {
            ...
            updateMonitor.setKeyguardOccluded(occluded);
            //*/
    ...
    public void onActivityDrawn() {
        /*/ freeme, gouzhouping. 20160806, for collapse the notification bar when click header which have notification on keyguard.
