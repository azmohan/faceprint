...
import com.droi.systemui.tool.BlurUtil;

import java.io.FileDescriptor;
import java.io.PrintWriter;
//*/freeme.zhangshaopiao,20170814,modify expand to ture for  swipe left and right action
...
//*/ Added by droi hanhao for custoimzed QSPanel 2016-01-08
    /*---------- Droi customized configurations ----------*/
    protected boolean mForceExpandedQS;
    /*-----------------------------------------*/
//*/
    ...
    //*/added by Droi shijiachen 20150718 for keyguard
    private boolean mIsEnableNotificationSlop = false;
    protected int mStatusBarState = StatusBarState.KEYGUARD;
    protected boolean mIsHeaderVisible = false;
    protected boolean mIsDefaultLockscreen = true;
    protected int mLastKeyguardStatus = StatusBarState.KEYGUARD;
    protected boolean mIsAnimatingExpandHeight = false;
    protected boolean mIsInstantCollapse = false;
    //*/

    protected BlurUtil mBlurUtil;
    protected boolean mHasBlured = false;
    protected boolean mShowBlurBackground = false;
    ...
    public PanelView(Context context, AttributeSet attrs) {
        ...
        //*/
        mBlurUtil = new BlurUtil(context);
    }

    /**
     * Init some customized configurations for Droi.
     * Added by droi hanhao
     * 2016-01-08
     */
    private void loadDroiConfigurations(Context context) {
        mForceExpandedQS = context.getResources().getBoolean(R.bool.config_force_expanded_qs);

        mShowBlurBackground = context.getResources().getBoolean(R.bool.droi_show_blur_background);

        if(DEBUG) {
            Log.d(TAG, "Customized configurations by droi {" + 
                        "mForceExpandedQS=" + mForceExpandedQS + 
                        "}");
        }
    }
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isFullyCollapsed() && !mHeadsUpManager.hasPinnedHeadsUp()) {
                    /*/ freeme, gouzhouping, 20161129, for remove the statusbar click slide result.
                    schedulePeek();
                    /*/
                }
                break;
    ...
    private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel) {
        ...
        if ((mTracking && mTouchSlopExceeded)
            ...
            //*/freeme.zhangshaopiao,20170814,modify expand to ture for add swipe left and right action
            if(SystemProperties.get("ro.freeme.xlj_jingdong").equals("1")){
                fling(vel, mStatusBar.getBarState() == StatusBarState.KEYGUARD ? true : expand, isFalseTouch(x, y));
                onTrackingStopped(mStatusBar.getBarState() == StatusBarState.KEYGUARD ? true : expand);
            }else{
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (scrolledToBottom || mTouchStartedInEmptyArea || mAnimatingOnDown) {
                    ...
                    if(mIsDefaultLockscreen){
                        ...
                        if ((h < -mTouchSlop || (mAnimatingOnDown && hAbs > mTouchSlop))
                                && hAbs > Math.abs(x - mInitialTouchX)&&(mStatusBar.getBarState() != StatusBarState.KEYGUARD)) {
                            cancelHeightAnimator();
                            startExpandMotion(x, y, true /* startTracking */, mExpandedHeight);
                            return true;
                        }
    ...
    protected void flingToHeight(float vel, boolean expand, float target,
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                //*/added by shijiachen 20150831, post delay to assure the height animate is already end.
