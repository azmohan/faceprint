...
public class IconMerger extends LinearLayout {
    ...
    private void checkOverflow(int width) {
        ...
        //*/ Added by tyd hanhao for customized more notifcation count shown, 2015-12-05
        int actrualShownCount = width / mIconSize;
        final int tempInvisibleCount = mTotal - actrualShownCount;
        //*/
        ...
        //*/ Added by tyd hanhao for customized more notifcation count shown, 2015-12-05
        if(tempInvisibleCount != invisibleCount) {
            post(new Runnable() {
                @Override
                public void run() {
                    if(mMoreView instanceof TextView) {
                        ((TextView)mMoreView).setText(
                                getResources().getString(R.string.keyguard_more_overflow_text, tempInvisibleCount));
                    }
                }
            }); 
            invisibleCount = tempInvisibleCount;
            if(DEBUG) {Log.d(TAG, "invisibleCount = " + invisibleCount);}
        }
        //*/
    }
