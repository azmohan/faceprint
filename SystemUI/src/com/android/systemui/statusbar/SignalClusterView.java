...
public class SignalClusterView
    ...
    private static boolean CUSTOMIZED; 
    ...
    ImageView mDataType, mDataActivityInOut;
    Map<Integer, int[]> mDataConn = new HashMap<Integer, int[]>();
    //*/
    ...
    public SignalClusterView(Context context, AttributeSet attrs, int defStyle) {
        ...
        CUSTOMIZED = context.getResources().getBoolean(R.bool.customized_signal_cluster_view);
        ...
        /// M: Add for Plugin feature @ {
        mStatusBarExt = PluginManager.getSystemUIStatusBarExtNew(context);
        /// @ }
    }
    ...
    protected void onAttachedToWindow() {
        ...
        mDataType = (ImageView) findViewById(R.id.data_type);
        mDataActivityInOut = (ImageView) findViewById(R.id.data_inout);
    ...
    public void setMobileDataIndicators(IconState statusIcon, IconState qsIcon, int statusType,
        ...
        int[] data = new int[]{statusType, dataActivity};
        
        mDataConn.put(subId, data);
        if(DEBUG) {
            Log.d(TAG, "set dataActivity {subId " + subId + ", moblieTypeId = " + statusType + ", dataActivity = " + dataActivity + "}");
        }
        ...
        /// M: Support "Operator plugin's ISignalClusterExt". @}
        //*/Add by droi lipengfei 20160116 for double signal
        if(statusIcon instanceof DroiIconState){
            if(DEBUG){
                Log.d(TAG, "statusIcon instanceof DroiIconState");
            }
            DroiIconState droiStatusIcon = (DroiIconState)statusIcon;
            state.mNetWorkTypeUpIcon = UP_NETWORK_ICON;
            state.mNetWorkTypeDownIcon = DOWN_NETWORK_ICON;
            state.mMobileUpIcon = droiStatusIcon.mobileUpIcon;
            state.mMobileDownIcon = droiStatusIcon.mobileDownIcon;
            state.mNetworkIcon = 0;
        }else{
            state.mNetWorkTypeUpIcon = 0;
            state.mNetWorkTypeDownIcon =0;
            state.mMobileUpIcon = 0;
            state.mMobileDownIcon = 0;
            state.mNetworkIcon = networkType;
        }
        //*/
    ...
    //*/Add by droi lipengfei 20160116 for double signal
    public static final int UP_NETWORK_ICON = R.drawable.droi_stat_sys_gemini_data_connected_4g_up;
    public static final int DOWN_NETWORK_ICON = R.drawable.droi_stat_sys_gemini_data_connected_4g_down;
    //*/
    ...
    public void setNoSims(boolean show) {
        ...
        mNoSimsVisible = PluginFactory.getStatusBarPlugin(mContext).customizeHasNoSims(
                mNoSimsVisible);
        //*/freeme,gouzhouping. 20160819, for 4G/3G icon disappear;
        if(mNoSimsVisible) {
            mDataConn.clear();
        }
        //*/
    ...
    public void onRtlPropertiesChanged(int layoutDirection) {
        ...
        for (PhoneState state : mPhoneStates) {
            ...
            //*/Add by droi lipengfei 20160116 for double signal
            if(state.mMobile2 != null){
                state.mMobile2.setImageDrawable(null);
            }
            if(state.mNetWorkTypeUp != null){
                state.mNetWorkTypeUp.setImageDrawable(null);
            }
            if(state.mNetWorkTypeDown != null){
                state.mNetWorkTypeDown.setImageDrawable(null);
            }
            //*/
        }
    ...
    private void apply() {
        ...
        for(Map.Entry<Integer, int[]> entry : mDataConn.entrySet()) {
            int subId = entry.getKey();
            int[] value = entry.getValue();
            if(DEBUG) {
                Log.d(TAG, "apply{subId = " + subId + ", mobileTypeId = " + value[0] + ", dataActivity = " + value[1]);
            }
            if(0 == value[0]) {
                continue;
            } else {
                mobileTypeId = value[0];
                dataActivity = value[1];
            }
        }
        
        if(DEBUG) {
            Log.d(TAG, "apply{final mobileTypeId = " + mobileTypeId + ", dataActivity = " + dataActivity + "}");
        }
        if(0 == mobileTypeId || mBlockMobile) {
            mDataType.setImageResource(0);
            mDataActivityInOut.setImageResource(0);
            mDataType.setVisibility(View.GONE);
            mDataActivityInOut.setVisibility(View.GONE);
        } else {
            mDataType.setImageResource(mobileTypeId);
            mDataActivityInOut.setImageResource(getDataActivityIcon(dataActivity));
    ...
    private void applyIconTint() {
        ...
        setTint(mEthernet, mIconTint);
        setTint(mVolteIcon, mIconTint);
        setTint(mDataType, mIconTint);
        setTint(mDataActivityInOut, mIconTint);
        //*/
    }
    ...
    private class PhoneState {
        ...
        private ImageView mNetWorkTypeUp;
        private ImageView mNetWorkTypeDown;
        private int mMobileUpIcon;
        private int mMobileDownIcon;
        private int mNetWorkTypeUpIcon;
        private int mNetWorkTypeDownIcon;
        //*/
        ...
        public PhoneState(int subId, Context context) {
            ...
            mPhoneStateExt = PluginManager.getSystemUIStatusBarExtNew(context);
        ...
        public void setViews(ViewGroup root) {
            ...
            //*/Add by droi lipengfei 20160116 for double signal
            mMobile2         = (ImageView) root.findViewById(R.id.mobile_signal2);
            mNetWorkTypeUp = (ImageView) root.findViewById(R.id.network_type_up);
            mNetWorkTypeDown = (ImageView) root.findViewById(R.id.network_type_down);
            //*/
        ...
        public boolean apply(boolean isSecondaryIcon) {
            if (mMobileVisible && !mIsAirplaneMode) {
                //*/Add by droi lipengfei 20160116 for double signal
                if(mMobileUpIcon != 0 && mMobileDownIcon != 0){
                    mMobile.setImageResource(mMobileUpIcon);
                    mMobile2.setImageResource(mMobileDownIcon);
                }else{
            ...
            mMobileGroup.setPaddingRelative(isSecondaryIcon ? mSecondaryTelephonyPadding : 0,
                    0, 0, 0);
            /*/Delete by droi lipengfei 20160116 for double signal
            ...
            //*/ Modified by droi hanhao for customized 2016-01-16
            if(CUSTOMIZED) {
        ...
        public void setIconTint(int tint, float darkIntensity) {
            ...
            setTint(mMobile2, tint);
            setTint(mNetWorkTypeUp, tint);
            setTint(mNetWorkTypeDown, tint);
            //*/
        ...
        /// @ }
        //*/Add by droi lipengfei 20160116 for double signal
        private void setDroiNetWorkIcon(){
            if(mNetWorkTypeUpIcon != 0 && mNetWorkTypeDownIcon != 0){
                mNetWorkTypeUp.setImageResource(mNetWorkTypeUpIcon);
                mNetWorkTypeDown.setImageResource(mNetWorkTypeDownIcon);
                mNetWorkTypeUp.setVisibility(View.VISIBLE);
                mNetWorkTypeDown.setVisibility(View.VISIBLE);
            }else{
                mNetWorkTypeUp.setVisibility(View.GONE);
                mNetWorkTypeDown.setVisibility(View.GONE);
            }
        }
        //*/
   ...
   /// M: Support "Operator plugin's ISignalClusterExt". @{
    ...
    //*/Add by droi lipengfei 20160116 for double signal
    static final int DATA_ACTIVITY_NONE = R.drawable.droi_stat_sys_signal_not_inout;
    static final int DATA_ACTIVITY_IN = R.drawable.droi_stat_sys_signal_in;
    static final int DATA_ACTIVITY_OUT = R.drawable.droi_stat_sys_signal_out;
    static final int DATA_ACTIVITY_INOUT = R.drawable.droi_stat_sys_signal_inout;
    //*/
