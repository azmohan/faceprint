...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    String[] mNetworkName;
    int mSlotCount = 0;
    /// For volte ALPS02516837, keep in migration
    int mImsSubId;
    int mImsRegState;
    /// @}
    
    //*/ Added by droi hanhao for customized 2016-01-18
    private ArrayList<PlmnCarrierListener> mPlmnListner = new ArrayList<PlmnCarrierListener>();
    //*/

    /// M: Support CT6M volte disconnected feature. @{
    ...
    private ContentObserver mImsSwitchChangeObserver  = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            ...
            sIsImsUseEnabledMap.put(defaultDataSubId, imsUseEnabled);
            Log.d(TAG, "mImsSwitchChangeObserver: onChange=" + selfChange +
                    ",subId: " + defaultDataSubId + ",ImsUseEnabled: " + imsUseEnabled);
        }
    };
    /// @}
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        mStatusBarSystemUIExt = PluginManager.getSystemUIStatusBarExtNew(mContext);
        /// M: Support for plugin. @}
    }
    ...
    private void unregisterListeners() {
        ...
        /// M: Support CT6M volte disconnected feature. @{
        mContext.getContentResolver().unregisterContentObserver(mImsSwitchChangeObserver);
        /// @}
    }
    ...
    //*/ Added by droi hanhao for customized 2016-01-18
    public void addPlmnCarrierListener(PlmnCarrierListener listener) {
        if(mPlmnListner.contains(listener)) {
            return;
        }
        
        mPlmnListner.add(listener);
    ...
    /// M: Support CT6M volte disconnected feature. @{
    private boolean isCtSim(int subId) {
        boolean ctSim = false;
    ...
    void doUpdateMobileControllers() {
        ...
        if (hasCorrectMobileControllers(subscriptions)) {
            ...
            for (MobileSignalController mobileSignalController : mMobileSignalControllers.values()) {
                mobileSignalController.updateNetworkNameExt();
            }
            //*/
            return;
        }
    ...
    //*/ Added by droi hanhao for customized 2016-01-18
    public interface PlmnCarrierListener {
        void updatePlmnCarrier(int slotId, boolean isSimInserted, boolean isHasSimService, String[] networkNames);
    }
    //*/
            ...
            if (subId == mImsSubId) {
                if(isImsOverWfc(intent) == true) {
                    ...
                    if (mImsRegState == ServiceState.STATE_IN_SERVICE &&
                        ...
                        iconId = getVolteIconId(phoneId);
                    } else if (FeatureOptionUtils.isMTK_CT6M_SUPPORT() && isCtSim(subId)) {
                        if (!signalController.getControllserHasService() ||
                                ((subId == SubscriptionManager.getDefaultDataSubId()
                                  && getImsUseEnabledStatus(subId))
                                 && (mImsRegState != ServiceState.STATE_IN_SERVICE))) {
                            iconId = getDisVolteIconId(phoneId);
                        } else {
                            iconId = 0;
                        }
                    } else {
    ...
    /// M: Support CT6M volte disconnected feature. @{
    public static final int[] DISVOLTEICON = {R.drawable.stat_sys_volte1_dis,
                                              R.drawable.stat_sys_volte2_dis};

    public int getDisVolteIconId(int slotId) {
        int iconId = 0;
        if (FeatureOptionUtils.isMTK_CT6M_SUPPORT()) {
            final TelephonyManager tm =
                    (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
            int slotCount = tm.getSimCount();
            iconId = R.drawable.stat_sys_volte_dis;
            if (slotCount > 1 && slotId < DISVOLTEICON.length) {
                iconId = DISVOLTEICON[slotId];
            }
        }
        return iconId;
    }
        ...
        for (int i = 0; i < mSlotCount; i++) {
            ...
            for (Map.Entry<Integer, MobileSignalController> entry : mMobileSignalControllers
                ...
                if (i == slotId) {
                    ...
                    //*/ Added by droi hanhao for customized 2016-01-18
                    for(PlmnCarrierListener listener : mPlmnListner) {
                        listener.updatePlmnCarrier(i, SIMHelper.isSimInserted(i), controller.getControllserHasService(), mNetworkName);
                    }
                    //*/
                    found = true;
                    break;
            ...
            if (!found) {
                ...
                //*/ Added by droi hanhao for customized 2016-01-18
                for(PlmnCarrierListener listener : mPlmnListner) {
                    listener.updatePlmnCarrier(i, false, false, mNetworkName);
                }
                //*/
            }
        }
