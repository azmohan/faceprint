...
public class FlashlightController {
    ...
    public void setFlashlight(boolean enabled) {
        Log.d(TAG, "setFlashlight -> " + enabled); // added log by droi hanhao for debug
        boolean pendingError = false;
        synchronized (this) {
            //*/ Modified by droi hanhao for limit flashlight in superPowerSaveMode 2016-03-14
            if (mFlashlightEnabled != enabled) {
                mFlashlightEnabled = enabled && !mPowerSaveMode;
                try {
                    mCameraManager.setTorchMode(mCameraId, mFlashlightEnabled);
    ...
    //*/ Added by droi hanhao for limit flashlight in superPowerSaveMode 2016-03-14
    public void setOnSuperPowerSaveMode(boolean powerSave) {
        boolean changed;
        synchronized (FlashlightController.this) {
            changed = mPowerSaveMode != powerSave;
            mPowerSaveMode = powerSave;
        }
        if (changed) {
            boolean enable = mFlashlightEnabled && !mPowerSaveMode;
            if (DEBUG) Log.d(TAG, "setFlashlight(" + enable + ") from superPowerSaveMode.");
