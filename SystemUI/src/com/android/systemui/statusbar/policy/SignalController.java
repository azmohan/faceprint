...
public abstract class SignalController<T extends SignalController.State,
    ...
    public int getDroiCurrentUpIconId(){
        if (mCurrentState.connected) {
            return TelephonyIcons.DROI_TELEPHONY_SIGNAL_STRENGTH_FOUR_G_UP[mCurrentState.level];
        } else if (mCurrentState.enabled) {
            return getIcons().mSbDiscState;
        } else {
            return getIcons().mSbNullState;
    ...
    public int getDroiCurrentDownIconId(){
        if (mCurrentState.connected) {
            return TelephonyIcons.DROI_TELEPHONY_SIGNAL_STRENGTH_FOUR_G_DOWN[getCsfbFakeSignalLevel(mCurrentState.level)];
        } else if (mCurrentState.enabled) {
            return getIcons().mSbDiscState;
        } else {
            return getIcons().mSbNullState;
    ...
    private static final int SIGNAL_CELL = 4;
    private int getCsfbFakeSignalLevel(int level) {
        return level > (SIGNAL_CELL - 2) ? SIGNAL_CELL : level + 2;
    }
    //*/
