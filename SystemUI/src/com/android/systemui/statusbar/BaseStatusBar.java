...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    protected boolean mIsDefaultLockscreen = true;
    //*/
  ...
  //*/Added by tyd hanhao, when super_power_save_enabled, disabled QsExpansion 2015-08-03
    protected boolean mSuperPowerSaveEnabled = false;
    //*/
    ...
    //*/ Added by tyd hanhao, when super_power_save_enabled, disabled QsExpansion 2015-08-03
    protected final ContentObserver mSuperPowerSaveSettingsObserver = new ContentObserver(mHandler) {
            @Override
            public void onChange(boolean selfChange) {
                final boolean enabled = 0 != Settings.System.getInt(
                    mContext.getContentResolver(), Settings.System.FREEME_SUPPER_POWER_SAVE_ENABLE, 0);
                if(DEBUG) {
                    Log.d(TAG, "super_power_save->" + enabled + ", old state ->" + mSuperPowerSaveEnabled);
                }
                if(mSuperPowerSaveEnabled != enabled) {
                    mSuperPowerSaveEnabled = enabled;
                    onSuperPowerSaveEnableChanged();
                }
            }
    };
    //*/
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
                ...
                if (MultiWindowProxy.isSupported()) {
                    ...
                    Log.d(TAG, "showFloatWindow is " + showFloatWindow);
                    mNavigationBarView.getFloatButton().setVisibility(showFloatWindow ?
                                                                     View.VISIBLE : View.INVISIBLE);
                    mNavigationBarView.getRestoreButton().setVisibility(showFloatWindow ?
                                                                     View.VISIBLE : View.INVISIBLE);
                }
                /// @}
    ...
    public void start() {
        ...
        //*/ Added by tyd hanhao, when super_power_save_enabled, disabled QsExpansion 2015-08-03
        mSuperPowerSaveSettingsObserver.onChange(false);
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(Settings.System.FREEME_SUPPER_POWER_SAVE_ENABLE), false,
                mSuperPowerSaveSettingsObserver);
        //*/
        ...
        //*/Modified by tyd Sean 20160111 for: use droi recents
        mRecents = getComponent(com.droi.recents.Recents.class);
        //*/
    ...
    protected SwipeHelper.LongPressListener getNotificationLongClicker() {
        return new SwipeHelper.LongPressListener() {
            ...
            public boolean onLongPress(View v, int x, int y) {
                ...
                //*/ Modified by tyd hanhao for customized notification's longClick event 2015-11-02
                /*/
                ...
                //*/
                dispatchNotificationLongClickEvent();
                //*/ Modified End.
                return true;
            }
        };
    ...
    protected boolean inflateViews(Entry entry, ViewGroup parent) {
        ...
        //*/ Added by tyd Jack 20131018 for, process notification icon by theme
        adjustContentView(sbn.getNotification(), contentView);
        adjustContentView(sbn.getNotification(), bigContentView);
        adjustContentView(sbn.getNotification(), headsUpContentView);
        //*/
        ...
        if (publicViewLocal == null) {
            ...
            if (entry.targetSdk >= Build.VERSION_CODES.LOLLIPOP
                ...
                //*/ Delete by tyd hanhao for process notification icon by theme 2015-08-04
                /*
        ...
        //*/ Delete by tyd hanhao for fix any notification bugs, 2015-08-27
    ...
    //*/ Added by Linguanrong for SystemUI new style, 2013-10-14, modified by tyd Jack 20131018 for, process notification icon by theme
    void adjustContentView(Notification notification, RemoteViews contentView) {
        if (contentView == null)
            return;
        ...
        if (notification.icon != 0) {
            ...
            try{
                contentView.setInt(com.android.internal.R.id.icon, "setBackgroundResource", 0);
                contentView.setViewPadding(com.android.internal.R.id.icon, 0, 0, 0, 0);
                 if(isFilterPackage(packageName) || isFilterApp(packageName)) {
                     if(notification.largeIcon == null) {
                         mIcon = pm.getIconBitmapWithThemeBg(
                             new BitmapDrawable(mContext.getResources(),
                                     rePaintIcon(mContext, packageName, notification.icon)), packageName, 0);
                         contentView.setImageViewBitmap(com.android.internal.R.id.icon, mIcon);
                     } else {
                         contentView.setImageViewBitmap(com.android.internal.R.id.icon, notification.largeIcon);
                         contentView.setImageViewResource(com.android.internal.R.id.right_icon, notification.icon);
                     }
                 } else {
                    ApplicationInfo appInfo = pm.getApplicationInfo(packageName, PackageManager.GET_META_DATA);
                    appIcon = pm.getApplicationIcon(appInfo);
                    if(appIcon != null) {
                        contentView.setImageViewBitmap(com.android.internal.R.id.icon, ((BitmapDrawable)appIcon).getBitmap());
                    } else {
                        contentView.setImageViewResource(com.android.internal.R.id.icon, notification.icon);
                    }
                }
    ...
    private static boolean isFilterPackage(String packagename) {
    ...
    private static boolean isFilterApp(String packagename) {
        if("com.tydtech.facewake".equals(packagename)) {
            return true;
        }
        return false;
    }

    private static Bitmap rePaintIcon(Context context, String packagename, int resid){
        float x, y;
        Bitmap iconBitmap = null;
        Bitmap dstbmp = null;
        
        Resources res = context.getResources();
        int mIconSize = res.getDimensionPixelSize(com.android.internal.R.dimen.scale_app_icon_size);
        ...
        if(isFilterApp(packagename)) {
            try {
                ...
                db = mPackageContext.getResources().getDrawable(resid);
            } catch (NameNotFoundException ne) {
                ne.printStackTrace();
            }
        }
        ...
        if(db instanceof BitmapDrawable) {
            iconBitmap = ((BitmapDrawable)db).getBitmap();
        } else if(db instanceof AnimationDrawable) {
            AnimationDrawable adb = (AnimationDrawable) db;
            iconBitmap = ((BitmapDrawable)(adb.getFrame(adb.getNumberOfFrames()-1))).getBitmap();
        } else {
            iconBitmap = Bitmap.createBitmap(
                    db.getIntrinsicWidth(),
                    db.getIntrinsicHeight(),
                    db.getOpacity() != PixelFormat.OPAQUE ?
        ...
        if(iconBitmap != null) {
            ...
            dstbmp = Bitmap.createBitmap(iconBitmap, 0, 0,
                        iconBitmap.getWidth(), iconBitmap.getHeight(), matrix, true); 
        } else {
            dstbmp = Bitmap.createBitmap(mIconSize, mIconSize, Config.ARGB_8888);
            dstbmp.eraseColor(Color.TRANSPARENT);
        }
        
        Bitmap bgBitmap = Bitmap.createBitmap(mIconSize, mIconSize, Config.ARGB_8888);
        ...
        x = (float) (mIconSize - dstbmp.getWidth()) / 2;
        y = (float) (mIconSize - dstbmp.getHeight()) / 2;
        canvas.drawBitmap(dstbmp, x, y, iconPaint);
    ...
    //*/ Added by tyd hanhao for disable active_state(click twice to open a notification) in normal keyguard 2015-11-26 
    protected void updateKeyguardRowStates(boolean forceActive) {
        ArrayList<Entry> activeNotifications = mNotificationData.getActiveNotifications();
        final int N = activeNotifications.size();
        for (int i = 0; i < N; i++) {
            NotificationData.Entry entry = activeNotifications.get(i);
            entry.row.makeForceActive(forceActive);
        }
    }
    //*/
     ...
     */
    protected void updateRowStates(boolean isForceExpand) { // modify  by Droi shijiachen for customize keyguard
        ...
        for (int i = 0; i < N; i++) {
            ...
            //*/ Added by tyd hanhao for disable active_state(click twice to open a notification) in normal keyguard 2015-11-26 
            entry.row.makeForceActive(!mIsDefaultLockscreen);
            //*/
            if (onKeyguard) {
            ...
            //*/ Added by tyd hanhao for show dimmed background in default lockscreen 2015-12-10
            entry.row.setDimmedBackground(onKeyguard && mIsDefaultLockscreen);
            //*/
    ...
    private void updateNotificationViews(Entry entry, StatusBarNotification notification) {
        ...
        //Moved about by freeme.chenming, 20160509. fixed bug for update notification by same notifyID
        Notification data = notification.getNotification();
        if(null != contentView) {
            adjustContentView(data, contentView);
        }
        if(null != bigContentView) {
            adjustContentView(data, bigContentView);
        }
        if(null != headsUpContentView) {
            adjustContentView(data, headsUpContentView);
        }
        if(null != publicContentView) {
            adjustContentView(data, publicContentView);
        }
        //*/
        ...
        /*/ Added by droi hanhao for customized notification icon,2016-01-22
        Notification data = notification.getNotification();
        if(null != bigContentView) {
            adjustContentView(data, bigContentView);
        }
