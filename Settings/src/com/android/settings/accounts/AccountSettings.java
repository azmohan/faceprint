...
public class AccountSettings extends SettingsPreferenceFragment
    ...
    //*/ Added by tyd liuchao for koobee acount, 2015-06-30
    private static final String KOOBEE_ACCOUNT_ACTION = "android.settings.ADD_KOOBEE_ACCOUNT_SETTINGS";
    //*/liuchao end

    //*/Added by TYD Sean 2015-08-06 for droi account begin
    private static final boolean DROI_ACCOUNT_ENABLED = true;
    //*/Added by TYD Sean 2015-08-06 for droi account end
    
   //*/ freeme. xiaocui,20160625. don't show droi account from contacts
    ...
    private static class ProfileData {
        ...
        //*/ Added by tyd liuchao for koobee acount, 2015-06-30
        public Preference koobeeAccountPreference;
        //*/liuchao end

        //*/Added by TYD Sean 2015-08-06 for droi account begin
        public Preference droiAccountPreference;
        //*/Added by TYD Sean 2015-08-06 for droi account end
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //*/ freeme .xiaocui,20160624. phone don't show droi account
        mFrom = getActivity().getIntent().getStringExtra("packageName");
        //*/
    ...
    public boolean onPreferenceClick(Preference preference) {
        //*/add by tyd wangalei 201510 14 for second menu sort
        CustomHobbyService mService=new CustomHobbyService(this.getActivity());
        if(mService.isExistData(R.string.account_settings_title, R.string.add_account_label)){
        mService.update(R.string.account_settings_title, R.string.add_account_label);
        }else{
            mService.insert(R.string.account_settings_title, R.string.add_account_label,"android.settings.ADD_ACCOUNT_SETTINGS", 1, "activity");
        }
        //*/
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (preference == profileData.addAccountPreference) {
                ...
                //*/ freeme. xiaocui,20160625. don't show droi account from contacts
                intent.putExtra("packageName", mFrom);
                //*/
                startActivity(intent);
                return true;
            }

            //*/Modified by TYD Sean 2015-08-06 for droi account begin
            if(DROI_ACCOUNT_ENABLED){
                if (preference == profileData.droiAccountPreference){
                    DroiAccount droiAccount = DroiAccount.getInstance(getActivity());
                    droiAccount.login();
                    return true;
                }

            }else if (preference == profileData.koobeeAccountPreference) {
                Intent intent = new Intent(KOOBEE_ACCOUNT_ACTION);
                startActivity(intent);
                return true;
            }
            //*/Modified by TYD Sean 2015-08-06 for droi account end
    ...
    private void updateProfileUi(final UserInfo userInfo, boolean addCategory,
        ...
        if (userInfo.isEnabled()) {
            ...
            if (!mUm.hasUserRestriction(DISALLOW_MODIFY_ACCOUNTS, userInfo.getUserHandle())) {
                //*/Modified by TYD Sean 2015-08-06 for droi account begin
                if(DROI_ACCOUNT_ENABLED){
                    //profileData.droiAccountPreference = newDroiAccountPreference(context);
                }else {
                    profileData.koobeeAccountPreference = newKoobeeAccountPreference(context);
                }
                //Modified by TYD Sean 2015-08-06 for droi account end
    ...
    //*/Added by tyd liuchao for koobee account, 20150714
    private Preference newKoobeeAccountPreference(Context context) {
        Preference preference = new Preference(context);
        preference.setTitle(R.string.login_account_koobee);
        preference.setIcon(R.drawable.ic_koobee_account);
        preference.setOnPreferenceClickListener(this);
    ...
    //*/Added by TYD Sean 2015-08-06 for droi account begin
    private Preference newDroiAccountPreference(Context context) {
        Preference preference = new Preference(context);
        preference.setTitle(R.string.login_account_droi);
        preference.setIcon(R.drawable.ic_droi_account);
        preference.setOnPreferenceClickListener(this);
        preference.setOrder(ORDER_First);
        return preference;
    }
    ...
    private void updateAccountTypes(ProfileData profileData) {
        ...
        if (profileData.userInfo.isEnabled()) {
            ...
            //*/ Added by tyd liuchao for koobee account, 2015-07-01
            if(DROI_ACCOUNT_ENABLED){
                if(profileData.droiAccountPreference != null){
                    profileData.preferenceGroup.addPreference(profileData.droiAccountPreference);
                }
            }else if (profileData.koobeeAccountPreference != null) {
                profileData.preferenceGroup.addPreference(profileData.koobeeAccountPreference);
            }
            //*/
            for (int i = 0; i < count; i++) {
                //*/Added by TYD Sean 2015-08-06 for droi account begin
                AccountPreference accountPreference = preferences.get(i);
                if(accountPreference.mFragmentArguments != null){
                    String accountType = accountPreference.mFragmentArguments.getString(ManageAccountsSettings.KEY_ACCOUNT_TYPE);
                    if(DroiAccount.ACCOUNT_TYPE.equals(accountType)){
                        //profileData.preferenceGroup.removePreference(profileData.droiAccountPreference);
                    }
                }
                //*/Added by TYD Sean 2015-08-06 for droi account end
    ...
    private ArrayList<AccountPreference> getAccountTypePreferences(AuthenticatorHelper helper,
        ...
        for (int i = 0; i < accountTypes.length; i++) {
            ...
            //*/droi.dukanglei,20170508,remove tiantiankuaibao in AccountType            
            if (titleResPackageName.equals("com.tencent.reading")){
                continue;
            }
            //*/
    ...
    new BaseSearchIndexProvider() {
        ...
        public List<SearchIndexableRaw> getRawDataToIndex(Context context, boolean enabled) {
            ...
            data.title = res.getString(R.string.account_settings_title);
            data.screenTitle = res.getString(R.string.account_settings_title);
            data.keywords = res.getString(R.string.account_settings_title);
            result.add(data);
