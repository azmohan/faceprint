...
public class ApnSettings extends SettingsPreferenceFragment implements
    ...
    private void fillList() {
        ...
        // 03139882 do not show ims apn for APTG, use hard code here
        if (!FeatureOption.MTK_VOLTE_SUPPORT || mccmnc.equals("52505")) {
            where += " AND NOT type='ims'";
        }
        /// @}

        /// M: for Telus, do not show emergency apn @{
        if (mccmnc.equals("302220") || mccmnc.equals("302221")) {
            where += " AND NOT type='emergency'";
        }
        /// @}
                ...
                if (selectable) {
                    ...
                    */
                    ...
                    if (FeatureOption.MTK_CT6M_SUPPORT) {
                        int subId = mSubscriptionInfo == null ?
                                null : mSubscriptionInfo.getSubscriptionId();
                        SvlteUiccUtils utils = SvlteUiccUtils.getInstance();
                        boolean isCtCard = utils.isRuimCsim(SubscriptionManager.getSlotId(subId));
                        int networkType = TelephonyManager.SIM_STATE_UNKNOWN;
                        try {
                        ...
                        if (mccmnc != null
                                && (mccmnc.equals(CT_NUMERIC_1) || mccmnc.equals(CT_NUMERIC_2))
                                && type != null
                                && !type.equals("ims")) {
                        ...
                        /// M: for CT volte feature, when sub is not default data card, 
                        /// or volte button off remove ims apn.
                        boolean isVolteSupport
                            = ImsManager.isVolteEnabledByPlatform(getActivity().getApplicationContext());
                        ...
                        if (mccmnc != null
                                && (mccmnc.equals(CT_NUMERIC_1) || mccmnc.equals(CT_NUMERIC_2))
                                && "ims".equals(type)) {
                            /// if volte not support or volte not checked, hide ims;
                            if (isChecked != 1 || !isVolteSupport || mainPhoneId != slotId) {
                                dataMccMnc = true;
                            }
                        ...
                        if (isCtCard && dataMccMnc) {
                            if (mnoMmsApnList != null && mnoMmsApnList.size() > 0) {
                                mnoMmsApnList.remove(mnoMmsApnList.size() - 1);
                            }
                        }

                        Log.d(TAG, "mccmnc = " + mccmnc + " isCtCard = " + isCtCard
    ...
    private String getSelectedApnKey() {
        ...
        int subId = mSubscriptionInfo.getSubscriptionId();
        Cursor cursor = getContentResolver().query(
                getPreferApnUri(FeatureOption.MTK_SVLTE_SUPPORT ? CdmaApnSetting.getPreferredSubId(
                        getActivity(), subId) : subId), new String[] { "_id" }, null, null,
    ...
    /**
     * M: SVLTE maintain 2 phone(CDMAPhone and LteDcPhone), need to set/get preferred apn using
     * correct sub id, LteDcPhone uses special sub id.
     * @param key
     * @param resolver
     * @param values
     */
    private void updatePreferApnForSvlte(String key, ContentResolver resolver,
            ContentValues values) {
        Cursor cursor = null;
        try {
            cursor = getContentResolver().query(Telephony.Carriers.CONTENT_URI,
                    new String[] { "_id", "sourcetype" }, "_id = " + key, null, null);

            int currentSub = mSubscriptionInfo.getSubscriptionId();
            Log.d(TAG, "setSelectedApnKey cursor.getCount() - " + cursor.getCount());
            if (cursor.getCount() > 0) {
                int c2kSlot = SvlteModeController.getActiveSvlteModeSlotId();
                Log.d(TAG, "setSelectedApnKey(), currentSub = " + currentSub + ", c2kSlot = "
                        + c2kSlot);
                int currentSlot = SubscriptionManager.getSlotId(currentSub);
                ...
                if (sourceType == 1 && c2kSlot == currentSlot) {
                    ...
                    resolver.update(getPreferApnUri(currentSub), values, null, null);
                    resolver.update(getPreferApnUri(SvlteUtils.getLteDcSubId(c2kSlot)), values,
                            null, null);
                } else {
                    ContentValues valuesNoPrefer = new ContentValues();
                    valuesNoPrefer.put(APN_ID, -1);
                    int preferredSub = CdmaApnSetting.getPreferredSubId(getActivity(), currentSub);
                    int lteDcSub = SvlteUtils.getLteDcSubId(c2kSlot);
                    Log.d(TAG, "setSelectedApnKey(), preferredSub =  " + preferredSub
                            + ", lteDcSub = " + lteDcSub);
                    resolver.update(getPreferApnUri(preferredSub), values, null, null);
                    resolver.update(getPreferApnUri(preferredSub == lteDcSub ? currentSub
                            : lteDcSub), valuesNoPrefer, null, null);
                }
            }
        } finally {
            if (cursor != null) {
