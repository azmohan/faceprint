...
public class WirelessSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    //*/Added by tyd liuchao, 2015-07-09
    ...
    private SwitchPreference mWirlessPreference;
    //*/
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (preference == mAirplaneModePreference && Boolean.parseBoolean(
            ...
            //*/Added by tyd_wangalei 2015.9.21 for Customs hobby sort
            CustomHobbyService mService=new CustomHobbyService(getActivity());
            if(mService.isExistData(R.string.radio_control_sim_title, R.string.network_settings_title)){
                mService.update(R.string.radio_control_sim_title, R.string.network_settings_title);
            }else{
                mService.insert(R.string.radio_control_sim_title, R.string.network_settings_title,"com.android.phone.MobileNetworkSettings", 1, "com.android.phone");
            }
            //*/
        }
        //*/freeme.zhangshaopiao,20170830.added for wireless toggle
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")) {
            if (mWirlessPreference != null && preference == mWirlessPreference && mWirlessPreference.isEnabled()) {
                mCm.setMobileDataEnabled(mWirlessPreference.isChecked());
            }
        }
        //*/
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       /// M: add the entrance RCS switch. @{
        ...
        //*/ Added by tyd liuchao for remove date usage setting
        getPreferenceScreen().removePreference(findPreference(KEY_DATA_USAGE_SETTINGS));
        //*/


        //*/freeme.zhangshaopiao,20170830.added for wireless toggle
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")) {
            mWirlessPreference = (SwitchPreference) findPreference("toggle_wireless");
        }
        //*/
        //*/freeme.fanwuyang,2017/10/16.remove for wireless toggle when isnot xiaolajiao_version
        if(!SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")) {
            getPreferenceScreen().removePreference(findPreference("toggle_wireless"));
        }
        //*/
    }
    ...
    public void onResume() {
        ...
        //*/freeme.chenruoquan, 20160830.added for hetcomm
        if (!UtilsExt.isPackageExist(getActivity(), UtilsExt.PKG_NAME_HETCOMM)){
        	removePreference("hetcom_settings");
        }
        //*/
        ...
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")) {
            ...
            if (simNum > 0 && (Settings.System.getInt(WirelessSettings.this.getContentResolver(), Settings.System.AIRPLANE_MODE_ON, 0) == 0)) {
                mWirlessPreference.setEnabled(true);
                //mWirlessPreference.setChecked(mCm.getMobileDataEnabled());
                mWirlessPreference.setChecked(mTm.getDataEnabled());
            } else {
                mWirlessPreference.setChecked(false);
                mWirlessPreference.setEnabled(false);
            }

            IntentFilter wirelessFilter = new IntentFilter();
            wirelessFilter.addAction(ConnectivityManager.CONNECTIVITY_ACTION);
            wirelessFilter.addAction("wirelessSettings.update");
            getActivity().registerReceiver(mWirelessReceiver, wirelessFilter);
        }
        //*/
    ...
    public void onPause() {
        ...
        //*/freeme.zhangshaopiao,20170830.added for wireless toggle
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")) {
            getActivity().unregisterReceiver(mWirelessReceiver);
        }
        //*/
    ...
    //*/freeme.zhangshaopiao,20170830.added for wireless toggle
    private final BroadcastReceiver mWirelessReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.d(TAG, "onReceive: "+intent.getAction());
            //*/freeme.shancong,20170831,set the value of data_connect_enabled to false if without SIMCard
            int simNum = SubscriptionManager.from(getActivity()).getActiveSubscriptionInfoCount();
            //*/
            if(mWirlessPreference != null && (Settings.System.getInt(WirelessSettings.this.getContentResolver(), Settings.System.AIRPLANE_MODE_ON, 0) == 1)){
                mWirlessPreference.setEnabled(false);
                return;
            }
            //*/freeme.shancong,20170831,set the value of data_connect_enabled to false if without SIMCard
            else if(mWirlessPreference != null && simNum <= 0){
                mWirlessPreference.setEnabled(false);
                return;
            }
            //*/
            else if (mWirlessPreference != null){
                mWirlessPreference.setEnabled(true);
            }
            if(mWirlessPreference != null && mWirlessPreference.isEnabled()){
                mWirlessPreference.setChecked(mTm.getDataEnabled());
            }
