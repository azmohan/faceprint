...
public class Index {
    ...
    private void updateInternal() {
        ...
        if(!isOOBEEnd() || !isSettingsActivity(mContext)){
            return;
        }
        //*/
        synchronized (mDataToProcess) {
    ...
    //*/Add by Tyd Jiangshouting 2015.11.19 for Setting search function lacking some data.
    private boolean isSettingsActivity(Context context){ 
        ActivityManager activityManager = 
            (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE); 
        List<ActivityManager.RunningTaskInfo> taskInfo = 
            activityManager.getRunningTasks(1);
        ComponentName componentInfo = taskInfo.get(0).topActivity;
        return componentInfo.getClassName().equals("com.android.settings.Settings");
    } 
    private boolean isOOBEEnd(){
        return 1 == android.provider.Settings.System.getInt(mContext.getContentResolver(),
            "freeme_oobe_state_finish" , 1);
    }
    //*/
