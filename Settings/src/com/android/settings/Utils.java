...
public final class Utils {
        ...
        if (intent != null) {
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                if ((resolveInfo.activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM)
                    ...
                    //*/ freeme.biantao, 20160404. for FreemeOTA.
                    if (listSize > 1 && !"com.freeme.ota".equals(resolveInfo.activityInfo.packageName)) {
                        continue;
                    }
                    //*/
    ...
    public static boolean updateTileToSpecificActivityOrRemove(Context context,DashboardTile tile, int flags) {
        ...
        if (intent != null) {
            ...
            for (int i = 0; i < listSize; i++) {
                ...
                if ((resolveInfo.activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM)
                    ...
                    if ((flags & UPDATE_PREFERENCE_FLAG_SET_TITLE_TO_MATCHING_ACTIVITY) != 0) {
                        // Set the preference title to the activity's label
                        tile.title = resolveInfo.loadLabel(pm);
                    }
   ...
   private static boolean isProfileOf(UserManager um, UserHandle otherUser) {
    ...
    //*/freeme.chenruoquan, 20160705.Added
    private static String[] mUninstalledPkgs;
    public static boolean isAppUninstalled(PackageInfo pkg) {
    	if(mUninstalledPkgs == null){
    		try{
            	mUninstalledPkgs = com.freeme.content.pm.FreemePackageManager.getDataPackagesForIgnoreUninstall();
            } catch(Exception e) {
            	return false;
            }
    	}
    	
    	int num = mUninstalledPkgs.length;
    	for(int i = 0; i < num; i++) {
    		if(pkg != null && mUninstalledPkgs[i].equals(pkg.packageName)) {
    			return true;
    		}
    	}
