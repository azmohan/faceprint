...
public class StorageVolumePreference extends Preference {
    ...
    public StorageVolumePreference(Context context, VolumeInfo volume, int color) {
        ...
        if (volume.isMountedReadable()) {
            ...
            /*/fremme.shancong,20171207,fix the value of storage
            final long totalBytes = path.getTotalSpace();
            //*/
            long relTotalSpace = 0;

            if (path.getTotalSpace() < 2L*1024*1024*1024){
                relTotalSpace = 2L*1024*1024*1024;
            }else if ((path.getTotalSpace() > 2L*1024*1024*1024) && (path.getTotalSpace() < 4L*1024*1024*1024)){
                relTotalSpace = 4L*1024*1024*1024;
            }else if ((path.getTotalSpace() > 4L*1024*1024*1024) && (path.getTotalSpace() < 8L*1024*1024*1024)){
                relTotalSpace = 8L*1024*1024*1024;
            }else if ((path.getTotalSpace() > 8L*1024*1024*1024) && (path.getTotalSpace() < 16L*1024*1024*1024)){
                relTotalSpace = 16L*1024*1024*1024;
            }else if ((path.getTotalSpace() > 16L*1024*1024*1024) && (path.getTotalSpace() < 32L*1024*1024*1024)){
                relTotalSpace = 32L*1024*1024*1024;
            }else{
                relTotalSpace = path.getTotalSpace();
            }

            final long totalBytes = relTotalSpace;
            //*/
