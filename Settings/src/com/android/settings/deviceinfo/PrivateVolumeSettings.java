...
public class PrivateVolumeSettings extends SettingsPreferenceFragment {
    ...
    //*/ freeme.menglingqiang, 20160929. show ram information in settings.
    private StorageSummaryPreference mRamSummary;
    //*/
    ...
    //*/ freeme.chenming, 20161105. modified internal storage total size
    private static final int DEFAULT_FLASH_SIZE = 16; // default flash size 16 GB
    //*/
    //*/ freeme.luyangjie,Integration of memory,2017.06.05
    private FreemeStorageInfoPreference mTotalStorage;
    private FreemeStorageInfoPreference mSystemStorage;
    private FreemeStorageInfoPreference mInternalStorage;
    private FreemeStorageInfoPreference mSysTotalStorage;
    private FreemeStorageInfoPreference mSysAvailableStorage;
    private int flag_layout_before = 0;
    private int flag_layout_middle = 1;
    private int flag_layout_after = 2;
    //*/
    ...
    public void onCreate(Bundle icicle) {
        ...
        //*/ freeme.menglingqiang, 20160929. show ram information in settings.
        mRamSummary = new StorageSummaryPreference(context);
        //*/
        //*/ freeme.luyangjie, 20170606. show Total Storage information in settings.
        mTotalStorage = new FreemeStorageInfoPreference(context, flag_layout_before);
        mSystemStorage = new FreemeStorageInfoPreference(context, flag_layout_middle);
        mInternalStorage = new FreemeStorageInfoPreference(context, flag_layout_after);
        mSysTotalStorage = new FreemeStorageInfoPreference(context, flag_layout_middle);
        mSysAvailableStorage = new FreemeStorageInfoPreference(context, flag_layout_middle);
        //*/
    ...
    public void update() {
        ...
        //*/ freeme.luyangjie, 20170606. show Total Storage information in settings.
        //*/ freeme.zhangshaopiao, 20170817. add ro.freeme.xlj_jingdong macro switch.
        //*/ freeme:fanwuyang on: 2017/10/17 remove ui modify for settings Storage show
        /*/
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1") && !SystemProperties.get("ro.freeme.xlj_jingdong").equals("1")) {
        //*/
        /*/
        ...
        //*/ freeme:fanwuyang on: 2017/10/17 remove ui modify for settings Storage show
        /*/
            addPreference(screen, mTotalStorage);
            addPreference(screen, mSystemStorage);
            addPreference(screen, mInternalStorage);
            addPreference(screen, mSysTotalStorage);
            addPreference(screen, mSysAvailableStorage);
        }else{
        //*/
            addPreference(screen, mSummary);
            addPreference(screen, mRamSummary);
        //*/ freeme:fanwuyang on: 2017/10/17 remove ui modify for settings Storage show
        /*/
        }
        //*/
        ...
        if (mVolume.fsUuid == StorageManager.UUID_PRIVATE_INTERNAL) {
            final long flashTotalBytes = android.os.SystemProperties.
                     getInt("ro.freeme.flash_size", DEFAULT_FLASH_SIZE) * GIGA_BYTE_SIZE;
            /*/freeme.shancong,20170808,make totalBytes to be compatible with different versions
            totalBytes = flashTotalBytes > file.getTotalSpace() ? flashTotalBytes : file.getTotalSpace();
            //*/
            if (file.getTotalSpace() < flashTotalBytes){
                totalBytes = flashTotalBytes;
            }else if ((file.getTotalSpace() > flashTotalBytes) && (file.getTotalSpace() < 16 * GIGA_BYTE_SIZE)){
                totalBytes = 16 * GIGA_BYTE_SIZE;
            }else if ((file.getTotalSpace() > 16 * GIGA_BYTE_SIZE) && (file.getTotalSpace() < 32 * GIGA_BYTE_SIZE)){
                totalBytes = 32 * GIGA_BYTE_SIZE;
            }
            //*/
        }
        //*/
        ...
        //*/ droi.duanzhiquan, 20161101. add chinese label for ram information in settings.
        mSummary.setTitle(TextUtils.expandTemplate(getText(R.string.droi_storage_rom_used)+" "+getText(R.string.storage_size_large),
                result.value, result.units));
        /*/
        mSummary.setTitle(TextUtils.expandTemplate(TextUtils.expandTemplate(getText(R.string.storage_size_large),
                result.value, result.units));
        //*/
        //*/ freeme.menglingqiang, 20160929. show ram information in settings.
        mSummary.setSummary(getString(R.string.freeme_storage_rom_used,
        /*/
        ...
        if ("1".equals(SystemProperties.get("ro.tyd_extreme_lk_support"))
                ...
                true)) {
            mAvailable = (long) (memInfo.availMem + (mTotalSize - memInfo.totalMem) * 0.5); //370=>550
            //fake rom and ram
            if (SystemProperties.get("ro.freeme.fake_rom_ram").equals("1")) {
            ...
            mAvailable = memInfo.totalMem - memInfo.availMem;
            mSweep = (float) (mAvailable / (1024 * 1024))
                    / (mTotalSize / (1024 * 1024));
        }
        //*/ droi.lizhaomei, 20160116. mofify ram information from Available to Used in settings.
        mRamSummary.setTitle(getText(R.string.droi_storage_ram_used)+" "+Formatter.formatShortFileSize(context, mTotalSize - mAvailable));
        /*/
        mRamSummary.setTitle(Formatter.formatShortFileSize(context, mAvailable));
        //*/
        mRamSummary.setSummary(getString(R.string.freeme_storage_ram_used,Formatter.formatFileSize(context, mTotalSize)));
        mRamSummary.setPercent((int)(((mTotalSize - mAvailable) * 100)/mTotalSize));
        //*/
        //*/freeme.luyangjie, 20170606. show Total Storage information in settings.
        mTotalStorage.setTitle(getText(R.string.total_storage));
        mTotalStorage.setSummary(Formatter.formatFileSize(context, totalBytes));
        mSystemStorage.setTitle(getText(R.string.system_storage));
        mSystemStorage.setSummary(Formatter.formatFileSize(context, (totalBytes - file.getTotalSpace())));
        mInternalStorage.setTitle(getText(R.string.Internal_storage));
        mInternalStorage.setSummary(Formatter.formatFileSize(context, file.getTotalSpace()));
        mSysTotalStorage.setTitle(getText(R.string.total_storages));
        mSysTotalStorage.setSummary(Formatter.formatFileSize(context, file.getTotalSpace()));
        mSysAvailableStorage.setTitle(getText(R.string.available_storage));
        mSysAvailableStorage.setSummary(Formatter.formatFileSize(context, file.getFreeSpace()));
        //*/

        //*/freeme.zhangshaopiao,20170817,modify xlj_jingdong store display mode
        if(SystemProperties.get("ro.freeme.xiaolajiao_version").equals("1")){
            mSummary.setTitle(TextUtils.expandTemplate(getText(R.string.droi_storage_rom_available)+" "+getText(R.string.storage_size_large),
                    Formatter.formatBytes(getResources(), freeBytes, 0).value, Formatter.formatBytes(getResources(), freeBytes, 0).units));
            mRamSummary.setTitle(getText(R.string.droi_storage_rom_available)+" "+Formatter.formatShortFileSize(context,mAvailable));
        }
        //*/
    ...
    private long getDisplayRamSize(long realSize) {
        ...
        //*/freeme.xuqian,20170906,for fake ram
    ...
    private void updateDetails(MeasurementDetails details) {
        for (int i = 0; i < mItemPoolIndex; ++i) {
            ...
            //*/ droi.liang.zhang show freeSpace replace otherSpace
            final long appSize = details.appsSize.get(userId);
            final long cacheSize = details.cacheSize;
            final long userSize = details.usersSize.get(userId);
	    	final long otherSize = details.miscSize.get(userId);
            ...
            final long freeSpace = (usedBytes - (imagesSize + videosSize + audioSize + appSize + cacheSize));

			//*/
            switch (itemTitleId) {
                case R.string.storage_detail_apps: {
                    updatePreference(item, appSize);
                } break;
                case R.string.storage_detail_images: {
                ...
                case R.string.storage_detail_other: {
                    if (SystemProperties.get("ro.freeme.other_contains_used").equals("1")) {
                        updatePreference(item, freeSpace);
                    } else {
                        updatePreference(item, otherSize);
                    }
                } break;
                case R.string.storage_detail_cached: {
                    updatePreference(item, cacheSize);
                } break;
                case 0: {
