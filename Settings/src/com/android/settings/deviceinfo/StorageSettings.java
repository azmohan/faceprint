...
public class StorageSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    private void refresh() {
        ...
        //*/freeme.shancong,20170825,fix the value of storage
        ...
        for (VolumeInfo vol : volumes) {
            if (vol.getType() == VolumeInfo.TYPE_PRIVATE) {
                ...
                if (vol.isMountedReadable()) {
                    ...
                    /*/freeme.shancong,20171207,fix the value of storage
                    privateUsedBytes += path.getTotalSpace() - path.getFreeSpace();
                    privateTotalBytes += path.getTotalSpace();
                    //*/
                    if (path.getTotalSpace() < 8L*1024*1024*1024){
                    ...
                    }else if ((path.getTotalSpace() > 32L*1024*1024*1024) && (path.getTotalSpace() < 64L*1024*1024*1024)){
                        relTotalSpace = 64L*1024*1024*1024;
                    }else{
                        relTotalSpace = path.getTotalSpace();
                    }
                    privateUsedBytes += relTotalSpace - path.getFreeSpace();
                    privateTotalBytes += relTotalSpace;
                    //*/
                }
