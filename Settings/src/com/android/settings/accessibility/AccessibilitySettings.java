...
public class AccessibilitySettings extends SettingsPreferenceFragment implements DialogCreatable,
    ...
    private void handleToggleLargeTextPreferenceClick() {
        ...
        //*/ Motify by jiangshouting 2015.08.21 for BigFont confusion.
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(getActivity());
        isFirstTime = prefs.getBoolean("isFirst",true);
        if(isFirstTime){
            prefs.edit().putBoolean("isFirst", false).commit();
            prefs.edit().putFloat("tyd_font_size", mCurConfig.fontScale).commit();
        }
        float fontSize = prefs.getFloat("tyd_font_size",1);
        try {
            //mCurConfig.fontScale = mToggleLargeTextPreference.isChecked() ? LARGE_FONT_SCALE : 1;
            mCurConfig.fontScale = mToggleLargeTextPreference.isChecked() ? LARGE_FONT_SCALE : fontSize;
        //*/
    ...
    private void initializeAllPreferences() {
        ...
        /*/Delete by Jiangshouting 2015.12.25 for setting code transplant
        // Captioning.
    ...
    private void updateServicesPreferences() {
        ...
        for (int i = 0, count = installedServices.size(); i < count; ++i) {
            ...
            ComponentName componentName = new ComponentName(serviceInfo.packageName,
                    serviceInfo.name);
            //*/ freeme.xuqian ,20170919 ,customize  market display
            if("com.zhuoyi.market".equals(serviceInfo.packageName)){
                 if(serviceInfo.name!=null&&serviceInfo.name.contains("AppAccessibilityService")){
                     title = getString(R.string.market_droi_easy_install) ;
                 }else if(serviceInfo.name!=null&&serviceInfo.name.contains("MonitorAccessibilityService")){
                     title = getString(R.string.market_droi_task_helper) ;
                 }
            }
            //*/
