...
public class InstalledAppDetails extends AppInfoBase
    ...
    private boolean handleDisableable(Button button) {
        ...
        /*/ freeme.swang merge 2017-01-09, Modified by Jack 20130419 for, disable "enable/disable button" when is system app
        return disableable;
        /*/
        Log.i("swang","app details = "+(button == mUninstallButton)+","+isSystemApp(mAppEntry.info.packageName)+","+disableable+","+mAppEntry.info.packageName);
        return (button == mUninstallButton) && isSystemApp(mAppEntry.info.packageName) ? false : disableable;
        //*/
    }

    //*/ freeme.swang ,2017-01-09 , add for get system app by packageInfo 
    public boolean isSystemApp(String packageName) {
        ApplicationInfo appInfo = null;
        PackageManager packageManager = getContext().getPackageManager();
        try {
            appInfo = packageManager.getApplicationInfo(packageName, PackageManager.GET_META_DATA);
            if ((appInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0 || (appInfo.flags == ApplicationInfo.FLAG_UPDATED_SYSTEM_APP)) {
                return true;
            }
    ...
    private void initUninstallButtons() {
        ...
        if (isBundled) {
            ...
            //*/freeme.chenruoquan, 20160705.Added to set uninstall button enabled false which in data
            if (Utils.isAppUninstalled(mPackageInfo)) {
            	enabled = false;
            }
            //*/
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        /// M: Launch button preference @ {
        IAppsExt appExt = UtilsExt.getAppsExtPlugin(getActivity());
        if (mAppEntry != null && mAppEntry.info != null) {
            appExt.launchApp(getPreferenceScreen(), mAppEntry.info.packageName);
        }
        /// @ }

        /// M: Set permission preference visibility depends on MoMs @ {
        if (!UtilsExt.isGmsBuild(getActivity()) && MobileManagerUtils.isSupported()) {
            removePreference(KEY_PERMISSION);
        }
        /// @ }
        /// M: System app and MTK app don't support runtime permission @ {
         if (!UtilsExt.isGmsBuild(getActivity()) && FeatureOption.MTK_RUNTIME_PERMISSION_SUPPORT) {
             if (mAppEntry != null && mAppEntry.info != null) {
                 if ((mAppEntry.info.flags & ApplicationInfo.FLAG_SYSTEM) != 0 &&
                         (mAppEntry.info.flagsEx & ApplicationInfo.FLAG_EX_OPERATOR) == 0) {
                     removePreference(KEY_PERMISSION);
                  }
             }
        }
        ...
        //*/Add by freeme wanbao.zhang for security center. ZYSE050301,2016031
        if (android.app.DroiPermissionManager.isSupported()) {
            removePreference(KEY_PERMISSION);
        }
        //*/
    }
